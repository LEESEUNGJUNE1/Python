...
# 분류 모델
dt_clf = DecisionTreeClassifier()   # sklearn의 다른 분류 모델을 써도 된다.
dt_clf.fit(train_data,train_label)
pred = dt_clf.predict(train_data)
print('예측값 : ',pred)
print('실제값 : ',train_label)
print('분류 정확도 : ',accuracy_score(train_label, pred)) # 순서 (실제값,예측값)
# 정확도 100% : 과적합 문제가 발생
...
print('\n--- 과적합 방지 목적 처리 2 ---')
# 교차검증 : train / test split을 했으나 여전히 과적합 -> 교차검증
# 본고사를 치르기 전에 모의고사를 여러 번 보는것
# 모델 학습시 데이터의 편중을 방지하고자 학습 데이터를 쪼개어 학습과 평가를 병행
# 교차검증 중 가장 보편적인 방법으로 k-fold가 있다.
...
for train_index, test_index in kfold.split(features):
    ...
    dt_clf.fit(xtrain,ytrain) # 학습을 120개로
    pred = dt_clf.predict(xtest) # 검증은 30개로
    n_iter += 1
    ...
    cv_acc.append(acc)
print('평균 검증 정확도 : ', np.mean(cv_acc)) # 0.9134


print('\n--- 과적합 방지 목적 처리 2-1 ---')
# 대출 사기 데이터인 경우 정상과 사기데이터가 비율적으로 편향된 경우가 많다. 이메일, 강우량 등등.. <- 불균형 데이터, 편향된 데이터
# 위와 같은 불균형 데이터인 경우는 KFold 보다는 StratifiedKFold를 많이 사용한다.
...
for train_index, test_index in skfold.split(features, label):
    train,xtest = features[train_index], features[test_index]
    ytrain,ytest = label[train_index],label[test_index]
    ...
    print('반복수 : {0}, validation data index : {1}'.format(n_iter, test_index))
    cv_acc.append(acc)
print('평균 검증 정확도 : ', np.mean(cv_acc)) # 0.77
...
score = cross_val_score(dt_clf, data, label, scoring='accuracy', cv = 5) # 모델,
...
# 여러 개의 파라미터(속성값)중 max_depth, min_samples_split 에 대해서 최적의 값 찾기
parameters = {'max_depth':[1, 2, 3], 'min_samples_split':[2, 3]}

grid_tree = GridSearchCV(dt_clf, param_grid = parameters, cv = 3, refit = True) # ,KFold,최적의 파라미터가 나올때 까지 재학습을해
