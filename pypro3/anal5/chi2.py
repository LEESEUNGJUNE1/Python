...
# data = [11, 5, 10, 13, 10, 11]
print(stats.chisquare(data))
result = stats.chisquare(data)
print('검정 통계량 X²값:%.5f, p-value:%.5f'%(result))
...
# 다시 말해서 검정에 사용된 data값은 우연히 발생된 값이 아니라 어떠한 필연적 원인에 의해 발생된 데이터라고 할 수 있다.
# 이 주사위는 게임에 적합하지 않다.

# 결과 얻기 2 - 카이제곱 분포표 사용
# 검정 통계량 X²값:14.20000, df : (N - 1)  # 5, 임계값 : 11.07 이므로 귀무가설을 기각하고 대립가설을 채택한다.
# 검정 통계량과 pvalue 값은 반비례관계이다.
