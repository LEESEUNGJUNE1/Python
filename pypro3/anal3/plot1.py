# 시각화 : 많은 양의 데이터를 효율적으로 봄으로써 인사이트를 정확하게 얻어낼수가 있다.

import numpy as np
import matplotlib.pyplot as plt
plt.rc('font',family='malgun gothic') # 한글이 안깨지게 설정 (음수가 표시가 안된다.)
plt.rcParams['axes.unicode_minus'] = False # 음수가 깨지는걸 방지

x = ["서울","인천","수원"] # set 순서가 없어서 X
y = [5, -3, 7]

# plt.plot(y)
#plt.xlim([-1,3]) # x,y 축 경계값 지정
#plt.ylim([0,10])
